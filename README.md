...
│   ├── storage/                # JSON 存储层
│   ├── utils/                  # JWT、密码等工具
│   ├── export_user_data.py     # 交互式服务器导出脚本
│   ├── main.py                 # 后端入口
│   └── requirements.txt
├── frontend/
│   ├── src/
...
├── zip/
│   └── user_exports/           # 导出任务目录、历史归档、task.json
...
- 管理后台导出走“创建任务 -> 轮询进度 -> 下载归档”流程；如果下载失败，优先检查前端当前来源是否被上面的 CORS 配置允许
...
- `memory/本体/`
- `backend/uploads/`
- `zip/user_exports/`（如需保留导出历史与可重复下载能力）
