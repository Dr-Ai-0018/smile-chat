...
const inferImageMimeFromBytes = (bytes) => {
...
const normalizeImageDataUrl = (image) => {
  ...
  if (commaIndex < 0) return image
  const header = image.slice(5, commaIndex)
  const base64 = image.slice(commaIndex + 1)
  const mime = normalizeImageMime(header.split(';', 1)[0])
  if (mime && mime !== 'application/octet-stream' && mime.startsWith('image/')) {
    return image
  }

  try {
    const binary = atob(base64.slice(0, 128))
    const bytes = new Uint8Array(binary.length)
    for (let i = 0; i < binary.length; i += 1) {
      bytes[i] = binary.charCodeAt(i)
    }
    const inferredMime = inferImageMimeFromBytes(bytes)
    if (inferredMime) {
...
const buildImageDataUrl = async (file) => {
  const buffer = await file.arrayBuffer()
  const bytes = new Uint8Array(buffer)
  const mime = (
    (() => {
      const normalized = normalizeImageMime(file.type)
      if (normalized && normalized !== 'application/octet-stream' && normalized.startsWith('image/')) {
        return normalized
      }
      return ''
    })() ||
    inferImageMimeFromBytes(bytes) ||
    inferImageMimeFromName(file.name) ||
    'image/jpeg'
  )
  ...
  try {
    ...
    if (response.history && response.history.length > 0) {
      messages.value = response.history.map(msg => ({
        ...
        role: msg.role,
        content: msg.content,
        image: normalizeImageDataUrl(msg.image || null),
...
const collectRecentMessages = () => {
  for (let i = messages.value.length - 1; i >= 0; i--) {
    ...
    if (msg.image && typeof msg.image === 'string') {
      result[0].image = normalizeImageDataUrl(msg.image) || msg.image
    }
...
const handleImageSelect = async (event) => {
  ...
  try {
    const base64 = await buildImageDataUrl(file)
